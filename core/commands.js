...
module.exports = function(bot) {
    ...
    bot.registerCommand(new RegExp(':taco:|:pizza:|:hotdog:|:burger:'), {hideHelp: true}, function(bot, params, message, slackbotCallback) {
        var resp = {};
        resp.text = 'Om nom nom!';
        slackbotCallback(null, resp);
    });

    bot.registerCommand('test', {alias: ['hello', 'hello?', 'hey', 'yo', 'hi'], hideHelp: true}, function(bot, params, message, slackbotCallback) {
        var resp = {};
        resp.text = 'I\'m up and running. What else do you want from me? Type `@'+bot.self.name+' help` to list available commands.';
        slackbotCallback(null, resp);
    });

    bot.registerCommand('sleep', {alias: ['zzz'], hideHelp: true}, function(bot, params, message, slackbotCallback) {
        var resp = {};
        resp.text = 'time for :bed: or... :coffee:';
        slackbotCallback(null, resp);
    });
